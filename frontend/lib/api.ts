...
export const api = {
  ...
  getChart: (symbol: string, range: string) =>
    request(`/api/stocks/${symbol}/chart?range=${range}`, () =>
      generateChart(symbol, range)
    ),

  getInsight: (symbol: string) =>
    request(`/api/ai/analysis/${symbol}`, () =>
      buildInsight(findStock(symbol))
    ),

  getQuiz: (symbol: string) =>
    request(`/api/quiz/${symbol}`, () =>
      buildQuiz(findStock(symbol))
    ),
};
